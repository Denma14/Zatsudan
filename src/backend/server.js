...
serverNet.on(events.C_Send_message, (socket, socketID, payload) => {
    console.log(payload);
    console.log("client's token is ", socket.token);
    const session = serverNet.clientIds.get(socket.token);
    if (!session)
        return;
    if (!payload.username)
        return;
...
serverNet.on(events.C_handshake, (socket, username, payload) => {
