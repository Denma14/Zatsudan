...
net.on(events.S_delete_spam_message, (payload: Types.deleteSpamPayload) => {
...
net.on(events.S_message_limit_error, (payload: Types.limitErrorPayload) => {
  displayError(payload);
});

net.on(events.S_error, (payload: any) => {
  displayError(payload);
});
//------------------------------------
net.init();
